...
class LoginForm(FlaskForm):
    login = StringField("Login Id",validators=[InputRequired(message='Login Id required')],render_kw={'Placeholder':'Login Id'})
...
class CustomerQueryForm(FlaskForm):
    customer_id = IntegerField("Customer Id",validators=[Optional(),NumberRange(min=100000000,max=999999999)],render_kw={'Placeholder':'Customer Id'})
    ssn = IntegerField("Customer SSN",validators=[Optional(),NumberRange(min=100000000,max=999999999)],render_kw={'Placeholder':'SSN'})
    submit = SubmitField("Submit")
